...
def main():
    conf = SparkConf()
    conf.setAppName('Release Optin User Locations By Release')
    ...
    df_tbc.join(df_ba, df_ba.artist_id == df_tbc.artist_id).select(
        ...
        df_tbc.latitude,
        df_tbc.optin_count
    ).write.mode("overwrite").parquet('/bit_daily/artist_dashboard/release_optin_user_locations_by_release')
